...
import { createDefaultStore } from './auth.token.store';
...
export const createAuthProvider = <T>({
  ...
  const geToken = () => {
    ...
    return [useAuth, authFetch, login, logout,geToken] as [typeof useAuth, typeof authFetch, typeof login, typeof logout,typeof geToken];
};
